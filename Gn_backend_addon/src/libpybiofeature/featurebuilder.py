...
from . import CKSAAP
from . import eighteen_physicochemical_properties as etpp
import tqdm
...
def build_pc_pse_aac(
    ...
    subprocess.run(f"python3 -u /mnt/md0/Tools/PseinOne/src {path_to_fasta} {data_dir}/SC-PseAAC.csv Protein SC-PseAAC -f csv -lamada 1 -w 0.5",shell=True, capture_output=True, text=True)
    df = pd.read_csv(f'{data_dir}/SC-PseAAC.csv',header=None)
    seq_list = list(SeqIO.parse(path_to_fasta, 'fasta'))
    df.index = [seq.id for seq in seq_list]
    return df.loc[seq_id_list, :]

def build_sc_pse_aac(
    data_dir:str,
    path_to_fasta: str,
    seq_id_list: list,
    desc: str = 'undefine',
    ...
    subprocess.run(f"python3 -u /mnt/md0/Tools/PseinOne/src {path_to_fasta} {data_dir}/PC-PseAAC.csv Protein PC-PseAAC -f csv -lamada 1 -w 0.5",shell=True, capture_output=True, text=True)
    df = pd.read_csv(f'{data_dir}/PC-PseAAC.csv',header=None)
    seq_list = list(SeqIO.parse(path_to_fasta, 'fasta'))
    df.index = [seq.id for seq in seq_list]
    return df.loc[seq_id_list, :]