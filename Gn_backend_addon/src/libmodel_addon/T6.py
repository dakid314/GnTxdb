...
def load_feature(
    ...
    feature_data_set.append({
        "name": "SA-100-TC",
        "submit": load_from_bert_tac(
            path=f"{data_dir}/sa_n.pkl",
            id_list=seq_id_list,
            dim=2
        ),
    })
...
def runmodel(
    ...
    threshold_dict = None
    with open(f"{os.environ['LIB_DIR']}/model/T6/threshold.json", "r", encoding="UTF-8") as f:
        threshold_dict = json.load(f)['threshold']

    with gzip.open(f"{os.environ['LIB_DIR']}/model/T6/model/scaler.pkl", "rb") as f:
        _scaler = pickle.load(f)

    result_dict = {}
    for model_name in model_selected_list:
        with gzip.open(f"{os.environ['LIB_DIR']}/model/T6/model/{model_name}.pkl", "rb", ) as f:
            model = pickle.load(f)
